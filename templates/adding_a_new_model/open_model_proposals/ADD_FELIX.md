How to add Felix to 🤗 Transformers?
=====================================
...
The following sections explain in detail how to add Felix 
to Transformers. You will work closely with patrick to
integrate Felix into Transformers. By doing so, you will both gain a 
theoretical and deep practical understanding of Felix. 
...
theoretical understanding of `Felix`. However, if you prefer to
understand the theoretical aspects of the model *on-the-job*, then it is
totally fine to directly dive into the `Felix`'s code-base. This
...
`Felix`'s paper, or if you just enjoy programming much more than
reading scientific papers.

### 1. (Optional) Theoretical aspects of Felix

You should take some time to read *Felix's* paper, if such
...
-   What type of model is *Felix*? BERT-like encoder-only
...
-   What are the applications of *Felix*? Text
...
#### Additional resources
 ...
 - [Felix Blog](https://ai.googleblog.com/2021/05/introducing-felix-flexible-text-editing.html)
 - [Paper](https://www.aclweb.org/anthology/2020.findings-emnlp.111.pdf)

#### Make sure you've understood the fundamental aspects of Felix

Alright, now you should be ready to take a closer look into the actual code of Felix.
You should have understood the following aspects of Felix by now:

- [characteristic 1 of Felix]
- [characteristic 2 of Felix]
- ...
...
4.  We recommend adding the PyTorch version of *Felix* to
...
5.  To port *Felix*, you will also need access to its
    original repository:

```bash
git clone https://github.com/google-research/google-research/tree/master/felix.git 
cd felix
pip install -e .
```

Now you have set up a development environment to port *Felix*
to 🤗 Transformers.
...
At first, you will work on the original *Felix* repository.
...
*Felix*. At Hugging Face, one of our main goals is to *make
...
You should start thereby by diving into the [original repository](https://github.com/google-research/google-research/tree/master/felix).
...
```python
model = FelixModel.load_pretrained_checkpoint("/path/to/checkpoint/")
...
6.  Retrieve the output of the whole Felix Model
...
*Felix* in which case an **efficient** debugging environment
...
#### More details on how to create a debugging environment for Felix 
...
Add *Felix*", in 🤗 Transformers so that you and the Hugging
...
5.  Once you are satisfied, go to the webpage of your fork on GitHub.
    Click on "Pull request". Make sure to add the GitHub handle of 
		patrick as a reviewer, so that the Hugging
...
**5. Adapt the generated models code for Felix**
...
`src/transformers/models/felix/modeling_felix.py` and
`src/transformers/models/felix/configuration_felix.py`.

Now you can finally start coding :). The generated code in
`src/transformers/models/felix/modeling_felix.py` will
...
`src/transformers/models/felix/modeling_felix.py`
...
implementation of *Felix*, *i.e.* the following command
should work:

```python
from transformers import FelixModel, FelixConfig
model = FelixModel(FelixConfig())
```

The above command will create a model according to the default
parameters as defined in `FelixConfig()` with random weights,
...
checkpoint you used to debug *Felix* in the original
repository to a checkpoint compatible with your just created 🤗
Transformers implementation of *Felix*. It is not advised to
...
framework as *Felix*. Usually, it is enough to copy an
...
config parameters in `FelixConfig()` that do not exactly match
...
either you used incorrect parameters in `FelixConfig()`, have a
...
[TODO FILL: Here the model name might have to be adapted, *e.g.*, maybe FelixForConditionalGeneration instead of FelixModel]

```python
model = FelixModel.from_pretrained("/path/to/converted/checkpoint/folder")
input_ids = [0, 4, 4, 3, 2, 4, 1, 7, 19]
...
under the same `tests/test_modeling_felix.py`. Run this test
file to verify that all common tests pass:

```python
pytest tests/test_modeling_felix.py
```
...
`FelixModelIntegrationTests` and only has to be filled out by
you. To ensure that those tests are passing, run

```python
RUN_SLOW=1 pytest -sv tests/test_modeling_felix.py::FelixModelIntegrationTests
```

**Note:** In case you are using Windows, you should replace `RUN_SLOW=1` with `SET RUN_SLOW=1`

Second, all features that are special to *Felix* should be
tested additionally in a separate test under
`FelixModelTester`/`FelixModelTest`. This part is often
...
Next, we should add the tokenizer of *Felix*. Usually, the
...
[TODO FILL: Here the mentor should add a comment whether a new tokenizer is required or if this is not the case which existing tokenizer closest resembles 
 Felix's tokenizer and how the tokenizer should be implemented]
 [...]
 [...]
...
For Felix, the tokenizer files can be found here:
...
```python
from transformers import FelixTokenizer
input_str = "This is a long example input string containing special characters .$?-, numbers 2872 234 12 and words."

tokenizer = FelixTokenizer.from_pretrained("/path/to/tokenizer/folder/")
...
Analogous to the modeling test files of *Felix*, the
tokenization test files of *Felix* should contain a couple of
...
`tests/test_modeling_felix.py` in 🤗 Transformers. Such a test
...
Now, all the necessary functionality for *Felix* is added -
...
`docs/source/model_doc/felix.rst` that you should fill out.
...
`src/transformers/models/felix/modeling_felix.py` is
...
Great, now you have added all the necessary code for *Felix*.
...
*Felix* can be used for inference and/or fine-tuned on a
